...
void BasicScene::Init(float fov, int width, int height, float near, float far)
{
	...
	decimationMult = 0.1; // the modifier to decimate with - change this to increase / decrease edge decimation

	// pre decimate mesh so user can cycle at runtime - change to false to run default decimation
	bool customPredecimate = true;
	
	PreDecimateMesh(sphereMesh, customPredecimate);
	PreDecimateMesh(cowMesh, customPredecimate);
	PreDecimateMesh(complexMesh, customPredecimate);
	PreDecimateMesh(cubeMesh, customPredecimate);

	// model creation for automotphing
	sphere1 = Model::Create("sphere", sphereMesh, material);
	cow = Model::Create("cow", cowMesh, material);
	...
	auto autoCow = AutoMorphingModel::Create(*cow, morphFunc);
